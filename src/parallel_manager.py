...
class DocumentQueue:
    ...
    def __init__(self, max_size: int = 20):
        ...
        logger.info(f"DocumentQueue initialized with max_size={max_size}")
    ...
    async def add_task(self, file_path: str, priority: int = 0) -> DocumentTask:
        ...
        async with self._lock:
            ...
            logger.info(f"Added task to queue: {task.filename} (priority={priority}, queue_size={len(self._queue)})")
            return task
        ...
        async with self._lock:
            ...
            for task in self._queue:
                if task.is_retryable:
                    ...
                    logger.info(f"Retrying task: {task.filename} (attempt {task.error_count + 1})")
                    return task
    ...
    async def complete_task(self, task: DocumentTask):
        ...
        async with self._lock:
            if task in self._queue:
                ...
                logger.info(f"Task completed: {task.filename} (status={task.status}, time={task.processing_time:.1f}s)")
...
class WorkflowDispatcher:
    ...
    def __init__(self, max_parallel_workflows: int = 2, workflow_timeout: float = 1800.0):
        ...
        logger.info(f"WorkflowDispatcher initialized with {max_parallel_workflows} slots")
    ...
    async def execute_task(
        ...
        async with self.semaphore:
            ...
            try:
                ...
                logger.info(f"Started processing {task.filename} in {slot.slot_id}")
                ...
                logger.info(f"Completed processing {task.filename} in {slot.slot_id}")
                return actual_result
                
            except Exception as e:
                error_msg = f"Workflow execution failed: {str(e)}"
                logger.error(f"Error processing {task.filename} in {slot.slot_id}: {error_msg}")
    ...
    def cleanup(self):
        ...
        logger.info("WorkflowDispatcher cleaned up")
...
class ProgressTracker:
    ...
    async def start_document_tracking(self, task: DocumentTask):
        ...
        async with self._lock:
            ...
            logger.debug(f"Started tracking progress for {task.filename}")
...
logger.info("Parallel manager module initialized successfully")