...
from .logging_config import log_document_start, log_document_stage, log_document_error
...
class DocumentProcessingWorkflow(Workflow):
    ...
    async def parse_document(self, ctx: Context, ev: DocumentLoadedEvent) -> DocumentParsedEvent | StopEvent:
        ...
        try:
            ...
            log_document_stage(filename, "parsing")
            ...
            # Log parsing error
            log_document_error(ev.filename, "parsing", str(e))
    ...
    async def extract_metadata(self, ctx: Context, ev: DocumentParsedEvent) -> MetadataExtractedEvent | StopEvent:
        ...
        try:
            ...
            filename = Path(ev.file_path).name
            log_document_stage(filename, "metadata extraction")
            ...
            filename = Path(ev.file_path).name
            log_document_error(filename, "metadata extraction", str(e))
    ...
    async def chunk_document(self, ctx: Context, ev: MetadataExtractedEvent) -> DocumentChunkedEvent | StopEvent:
        ...
        try:
            ...
            filename = Path(ev.file_path).name
            log_document_stage(filename, "chunking", f"{len(chunks)} chunks")
            ...
            # Log chunking error
            filename = Path(ev.file_path).name
            log_document_error(filename, "chunking", str(e))
    ...
    async def index_chunks(self, ctx: Context, ev: DocumentChunkedEvent) -> StopEvent:
        ...
        try:
            ...
            filename = Path(ev.file_path).name
            log_document_stage(filename, "indexing")
            ...
            # Log indexing error
            filename = Path(ev.file_path).name
            log_document_error(filename, "indexing", str(e))
