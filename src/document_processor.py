...
def _run_async_safely(coro):
    ...
    try:
        ...
        logger.warning(f"Event loop issue detected: {e}. Creating new event loop.")
...
class DocumentProcessor:
    ...
    def __init__(self, config: Optional[DocumentProcessorConfig] = None):
        ...
        logger.info(f"DocumentProcessor initialized with result_type: {self.config.result_type}")
    ...
    async def process_pdf_async(self, file_path: Union[str, List[str]]) -> List[Document]:
        ...
        """
        try:
            logger.info(f"Async processing PDF: {file_path}")
            ...
            logger.info(f"Successfully processed {len(documents)} documents (async)")
            ...
            logger.error(f"Error processing PDF {file_path} (async): {str(e)}")
    ...
    def get_json_result(self, file_path: str) -> List[dict]:
        ...
        """
        try:
            logger.info(f"Getting JSON result for: {file_path}")
            
            json_results = self.parser.get_json_result(file_path)
            
            logger.info(f"Successfully extracted JSON for {len(json_results)} document(s)")
            
            return json_results
            
        except Exception as e:
            logger.error(f"Error getting JSON result for {file_path}: {str(e)}")
    ...
    def get_images(self, json_results: List[dict], download_path: str = "extracted_images") -> List[dict]:
        ...
        try:
            ...
            logger.info(f"Successfully extracted {len(image_dicts)} images")
    ...
    def create_documents_from_json(self, json_results: List[dict]) -> List[Document]:
        ...
        logger.info(f"Created {len(documents)} page-level documents from JSON")
    ...
    def validate_processing_result(self, documents: List[Document]) -> bool:
        ...
        for i, doc in enumerate(documents):
            ...
            if len(doc.text.strip()) < 10:
                logger.warning(f"Document {i} has very short content")
                return False
            
            # Check for metadata presence
            if not hasattr(doc, 'metadata') or not doc.metadata:
                logger.warning(f"Document {i} missing metadata")
                return False
        
        logger.info(f"Validation passed for {len(documents)} documents")
        return True
...
class ArxivDocumentProcessor:
    ...
    def process(self, file_path: Union[str, List[str]]) -> dict:
        ...
        """
        try:
            logger.info(f"Starting end-to-end processing for: {file_path}")
