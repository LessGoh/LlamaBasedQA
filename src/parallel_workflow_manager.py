...
def _ensure_event_loop():
    ...
    try:
        ...
        logger.warning("Creating new event loop for parallel processing")
...
class ParallelWorkflowManager:
    ...
    def __init__(self, config: Optional[BatchProcessingConfig] = None):
        ...
        logger.info(f"ParallelWorkflowManager initialized with config: {self.config}")
    ...
    def _safe_async_run(self, coro):
        ...
        try:
            ...
            logger.warning(f"Event loop issue in parallel processing: {e}")
    ...
    async def start_batch_processing(
        ...
        try:
            ...
            logger.info(f"Batch processing started: batch_id={batch_id}")
    ...
    async def stop_processing(self, wait_for_completion: bool = True) -> BatchProcessingResult:
        ...
        if self.status not in [QueueStatus.RUNNING, QueueStatus.STOPPING]:
            logger.warning(f"Cannot stop processing: status is {self.status}")
            return self.current_batch
        
        logger.info("Stopping batch processing gracefully...")
        ...
        try:
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    logger.info(f"Completed document: {task.filename}")
                    
                except Exception as e:
                    logger.error(f"Error processing document {task.filename}: {str(e)}")
    ...
    async def _progress_update_loop(self):
        ...
        try:
            ...
            logger.info("Progress update loop cancelled")
        except Exception as e:
            logger.error(f"Error in progress update loop: {str(e)}")
    ...
    async def _wait_for_completion_or_stop(self):
        ...
        while not self.stop_event.is_set():
            ...
            if (queue_stats["pending"] == 0 and 
                ...
                queue_stats["error"] == 0):
                logger.info("All documents processed, stopping")
                break
    ...
    async def _finalize_batch(self):
        ...
        logger.info(f"Batch processing finalized: {self.current_batch.to_dict()}")
    ...
    def cleanup(self):
        ...
        logger.info("ParallelWorkflowManager cleaned up")
...
logger.info("Parallel workflow manager module initialized successfully")