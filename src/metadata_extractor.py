...
class MetadataExtractor:
    ...
    def __init__(self, config: Optional[MetadataExtractorConfig] = None):
        ...
        logger.info(f"MetadataExtractor initialized with mode: {self.config.extraction_mode}")
    ...
    def _initialize_llama_extract(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "LlamaExtract library not found. "
            ...
            logger.warning(f"Failed to initialize LlamaExtract: {str(e)}")
            self._use_fallback_implementation()
    ...
    def _get_or_create_agent(self, agent_name: str, pydantic_schema):
        ...
        try:
            ...
            logger.info(f"Checking for existing agent: {agent_name}")
            ...
            try:
                ...
                logger.info("Unable to list existing agents, proceeding with creation attempt")
            except Exception as e:
                logger.warning(f"Error checking existing agents: {str(e)}")
            ...
            if "409" in str(e) or "already exists" in str(e).lower():
                logger.info(f"Agent {agent_name} already exists, attempting to retrieve it")
                ...
                try:
                    ...
                    if hasattr(self.extractor, 'get_agent'):
                        ...
                        logger.info(f"Successfully retrieved existing agent: {agent_name}")
                        return agent
                    else:
                        # If no direct get method, try to use the agent anyway
                        # This is a workaround - we'll create a mock agent reference
                        logger.warning(f"No get_agent method available, using fallback approach")
                        ...
                        logger.info(f"Using existing agent via fallback approach: {agent_name}")
                        return agent
                        
                except Exception as get_error:
                    logger.error(f"Failed to retrieve existing agent: {str(get_error)}")
                    raise Exception(f"Agent exists but cannot be retrieved: {str(get_error)}")
            else:
                # Some other error occurred
                logger.error(f"Failed to create or get agent: {str(e)}")
    ...
    def _use_fallback_implementation(self):
        ...
        try:
            ...
            logger.info("Using fallback LlamaIndex extractors")
            
        except Exception as e:
            logger.error(f"Failed to initialize fallback extractors: {str(e)}")
            raise
    ...
    def _extract_with_llama_extract(self, document: Union[Document, str]) -> ExtractedMetadata:
        ...
        try:
            ...
            logger.info(f"Waiting {self.config.delay_seconds} seconds before LlamaExtract call to reduce API load...")
            ...
            logger.error(f"LlamaExtract extraction failed: {str(e)}")
            raise
    ...
    def _extract_with_fallback(self, document: Union[Document, str]) -> ExtractedMetadata:
        ...
        try:
            ...
            logger.info("Fallback metadata extraction completed")
            return metadata
    ...
    def extract_batch(self, documents: List[Union[Document, str]]) -> List[ExtractedMetadata]:
        ...
        for i, doc in enumerate(documents):
            try:
                ...
                logger.error(f"Failed to extract metadata from document {i+1}: {str(e)}")
